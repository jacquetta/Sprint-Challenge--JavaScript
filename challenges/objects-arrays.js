...
zooAnimals.forEach(function(zooAnimal){
  animalNames.push(`First: ${zooAnimal.animal_name} Scientific: ${zooAnimal.scientific_name}`);
});
...
zooAnimals.map((zooAnimal) => {
  return lowerCase.push({'Name': zooAnimal.animal_name.toLowerCase()});
});
...
largerPopulation.push(zooAnimals.filter((zooAnimal) => {
...
*/
const totalAnimals= zooAnimals.reduce((total, zooAnimal) => {
  return total += zooAnimal.population;
}, 0);
